...
from ..united_system.float_array import FloatArray as InternalFloatArray
from .bool_array import BoolArray

@dataclass(frozen=True, slots=True, init=False)
class FloatArray(JSONable, HDF5able, Sized):
    ...
    def __init__(self, values: Union[np.ndarray[Any, Any], list[Any], InternalFloatArray]):
        """
        ...
        match values:
            ...
            case np.ndarray() | list():
                ...
                if values.ndim != 1:
                    raise ValueError("FloatArray must be 1-dimensional")
    ...
    def __getitem__(self, index_or_slice: Union[int, slice]) -> Union[float, "FloatArray"]:
        ...
        result: Union[float, InternalFloatArray] = self._wrapped_array[index_or_slice]
    ...
    # Factory methods
    @classmethod
    def zeros(cls, shape: Union[int, tuple[int, ...]]) -> "FloatArray":
        """Create an array of zeros."""
        values = np.zeros(shape, dtype=float)
        return cls(values)

    @classmethod
    def ones(cls, shape: Union[int, tuple[int, ...]]) -> "FloatArray":
        """Create an array of ones."""
        values = np.ones(shape, dtype=float)
        return cls(values)

    @classmethod
    def full(cls, shape: Union[int, tuple[int, ...]], fill_value: float) -> "FloatArray":
    ...
    @classmethod
    def random(cls, shape: Union[int, tuple[int, ...]], low: float = 0.0, high: float = 1.0) -> "FloatArray":
        """Create random values."""
        values = np.random.uniform(low, high, shape)
        return cls(values)

    @classmethod
    def from_values(cls, values: Union[np.ndarray[Any, Any], list[Any]]) -> "FloatArray":
        """Create from values."""
        return cls(values)
    ...
    @classmethod
    def from_json(cls, data: dict[str, Any], **_: Any) -> "FloatArray":
        """Create from JSON dictionary."""
        return cls(InternalFloatArray.from_json(data)) # type: ignore
    ...
    @classmethod
    def from_hdf5(cls, hdf5_group: h5py.Group, **_: Any) -> "FloatArray":
        """Create from HDF5 group."""
        return cls(InternalFloatArray.from_hdf5(hdf5_group)) # type: ignore