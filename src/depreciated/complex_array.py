...
from typing import Union, Any, Tuple, Sized
import numpy as np
import h5py
from ..united_system.utils.general import JSONable, HDF5able
from ..united_system.complex_array import ComplexArray as InternalComplexArray
from .bool_array import BoolArray
from .float_array import FloatArray

@dataclass(frozen=True, slots=True, init=False)
class ComplexArray(JSONable, HDF5able, Sized):
    ...
    def __init__(self, values: Union[np.ndarray[Any, Any], list[Any], InternalComplexArray]):
        """
        ...
        match values:
            ...
            case np.ndarray() | list():
                ...
                if values.ndim != 1:
                    raise ValueError("ComplexArray must be 1-dimensional")
    ...
    def __getitem__(self, index_or_slice: Union[int, slice]) -> Union[complex, "ComplexArray"]:
        ...
        result: Union[complex, InternalComplexArray] = self._wrapped_array[index_or_slice]
        if isinstance(result, (float, int, complex, np.floating, np.integer)):
    ...
    def __rsub__(self, other: Union["ComplexArray", complex, float, int, np.ndarray]) -> "ComplexArray":
        """Reverse subtraction."""
        if isinstance(other, ComplexArray):
            result = other.values - self.values
        else:
    ...
    def __rtruediv__(self, other: Union["ComplexArray", complex, float, int, np.ndarray]) -> "ComplexArray":
        """Reverse division."""
        if isinstance(other, ComplexArray):
            result = other.values / self.values
        else:
    ...
    # Factory methods
    @classmethod
    def zeros(cls, shape: Union[int, tuple[int, ...]]) -> "ComplexArray":
        """Create an array of zeros."""
        values = np.zeros(shape, dtype=complex)
        return cls(values)

    @classmethod
    def ones(cls, shape: Union[int, tuple[int, ...]]) -> "ComplexArray":
        """Create an array of ones."""
        values = np.ones(shape, dtype=complex)
        return cls(values)

    @classmethod
    def full(cls, shape: Union[int, tuple[int, ...]], fill_value: complex) -> "ComplexArray":
        """Create an array filled with a value."""
        values = np.full(shape, fill_value, dtype=complex)
        return cls(values)

    @classmethod
    def from_real_imag(cls, real: Union[np.ndarray[Any, Any], list[Any]], imag: Union[np.ndarray[Any, Any], list[Any]]) -> "ComplexArray":
    ...
    @classmethod
    def from_polar(cls, magnitude: Union[np.ndarray[Any, Any], list[Any]], phase: Union[np.ndarray[Any, Any], list[Any]]) -> "ComplexArray":
    ...
    @classmethod
    def random(cls, shape: Union[int, tuple[int, ...]]) -> "ComplexArray":
    ...
    @classmethod
    def from_values(cls, values: Union[np.ndarray[Any, Any], list[Any]]) -> "ComplexArray":
        """Create from values."""
        return cls(values)
    ...
    @classmethod
    def from_json(cls, data: dict[str, Any], **_: Any) -> "ComplexArray":
        """Create from JSON dictionary."""
        return cls(InternalComplexArray.from_json(data)) # type: ignore
    ...
    @classmethod
    def from_hdf5(cls, hdf5_group: h5py.Group, **_: Any) -> "ComplexArray":
        """Create from HDF5 group."""
        return cls(InternalComplexArray.from_hdf5(hdf5_group)) # type: ignore