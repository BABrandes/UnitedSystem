...
class SimpleUnit(Unit):
    ...
    def compatible_to(self, other: "Unit|UnitQuantity") -> bool:
        if isinstance(other, UnitQuantity):
            ...
            return self.unit_quantity == other.unit_quantity
        else:
    ...
    def suggest_unit_from_named_units(quantity: SimpleCanonicalQuantity, canonical_value: float|None) -> "SimpleUnit":
        ...
        for exponent in exponents_to_check:
            named_canonical_quantity: NamedSimpleUnitQuantity|None = NamedSimpleUnitQuantity.find_named_canonical_quantity(quantity / exponent)
        ...
        if canonical_value is not None:
            named_units: set[NamedUnit] = NamedUnit.find_named_units(canonical_unit.unit_quantity)
    ...
    def suggest_units(quantity: SimpleCanonicalQuantity, value: float|None, must_include: set["SimpleUnit"]|list["SimpleUnit"]=set(), n: int = 1000) ->  Tuple["SimpleUnit", list["SimpleUnit"]]:
        ...
        for exponent in exponents_to_check:
            named_canonical_quantity: NamedSimpleUnitQuantity|None = NamedSimpleUnitQuantity.find_named_canonical_quantity(quantity_to_get_to_zero / exponent)
            if named_canonical_quantity is not None:
        ...
        if quantity_to_get_to_zero.is_zero():
            ...
            for unit in units:
                options[unit] = set()
                named_canonical_quantity: NamedSimpleUnitQuantity|None = NamedSimpleUnitQuantity.find_named_canonical_quantity(unit.unit_quantity)
                if named_canonical_quantity is not None:
                    for named_unit in NamedUnit.find_named_units(unit.unit_quantity):
