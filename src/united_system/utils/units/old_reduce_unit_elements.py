...
def reduce_unit_elements(elements: Sequence[UnitElement]) -> Sequence[UnitElement]:
    ...
    """
    print(f"DEBUG: reduce_unit_elements called with {len(elements)} elements")
    ...
    def update_final_named_quantities_candidates(
        ...
        for named_quantity in CONSIDER_FOR_REDUCTION_QUANTITIES:
            # Debug: Only print when pursuing POWER (W)
            if named_quantity == NamedQuantity.POWER:
                current_str = " + ".join([f"{q.value[0]}^{exp}" if exp != 1 else q.value[0] for q, exp in current_named_quantities]) if current_named_quantities else "empty"
                print(f"DEBUG: Pursuing POWER (W). Current: [{current_str}], Remaining: {remaining_proper_exponents}")
            
            for index in range(8):
                if named_quantity.proper_exponents_as_tuple[index] == 0:
                    continue
                exponent, new_proper_exponents = subtract(index, remaining_proper_exponents, named_quantity.value[1].proper_exponents)
    ...
    proper_exponents: tuple[float, float, float, float, float, float, float, float] = ProperExponents.proper_exponents_of_unit_elements(elements)
    final_named_quantities_candidates: list[list[tuple[NamedQuantity, float]]] = []
    update_final_named_quantities_candidates(
        [],
        proper_exponents,
