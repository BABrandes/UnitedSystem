...
from typing import Generic, Any, Optional, TypeVar, Union, Sequence, Iterator, Callable, overload, Iterable, Type, Mapping, Literal
...
class UnitedDataframeProtocol(Generic[CK, T]):
    ...
    @classmethod
    def _construct(cls, dataframe: pd.DataFrame, column_keys: Sequence[CK], column_types: Mapping[CK, ColumnType], column_units: Mapping[CK, Optional[Unit]], internal_dataframe_column_name_formatter: InternalDataFrameColumnNameFormatter=SimpleInternalDataFrameNameFormatter(), read_only: bool = False, copy_dataframe: bool = False, rename_dataframe_columns: bool = False) -> "UnitedDataframe[CK]": ...
    ...
    def _cell_set_scalar(self, row_index: int, column_key: CK, value: SCALAR_TYPE): ...
    def _cell_set_value(self, row_index: int, column_key: CK, value: VALUE_TYPE, unit: Optional[Unit]=None) -> None: ...
    def _cell_set_item(self, row_index: int, column_key: CK, value: SCALAR_TYPE|VALUE_TYPE) -> None: ...
    ...
    def _mask_get_duplicates(self, subset: list[CK] | None = None, keep: Literal["first", "last"] = "first") -> BoolArray: ...
    def _mask_remove_duplicates(self, subset: list[CK] | None = None, keep: Literal["first", "last"] = "first", inplace: Literal[False, True] = False) -> Optional["UnitedDataframe[CK]"]: ...
    ...
    def _row_set_ordered(self, row_index: int, items: Sequence[VALUE_TYPE|SCALAR_TYPE]) -> None: ...
    ...
    def row_get_as_dict(self, row_index: int, column_keys: Sequence[CK]|None = None) -> Mapping[CK, VALUE_TYPE|SCALAR_TYPE]: ...
    def row_get_head(self, n: int = 5) -> Sequence[Mapping[CK, VALUE_TYPE|SCALAR_TYPE]]: ...
    def row_get_tail(self, n: int = 5) -> Sequence[Mapping[CK, VALUE_TYPE|SCALAR_TYPE]]: ...
    def row_get_first(self) -> Mapping[CK, VALUE_TYPE|SCALAR_TYPE]: ...
    def row_get_last(self) -> Mapping[CK, VALUE_TYPE|SCALAR_TYPE]: ...
    def row_get_by_slice(self, start: int, stop: int, step: int = 1) -> list[Mapping[CK, VALUE_TYPE|SCALAR_TYPE]]: ...
    ...
    def mask_get_duplicates(self, subset: list[CK] | None = None, keep: Literal["first", "last"] = "first") -> BoolArray: ...
    def mask_remove_duplicates(self, subset: list[CK] | None = None, keep: Literal["first", "last"] = "first", inplace: Literal[False, True] = False) -> Optional["UnitedDataframe[CK]"]: ...
    ...
    @classmethod
    def from_hdf5(cls, group: Union[h5py.Group, tuple[str, h5py.Group]], column_key_type: Type[CK], internal_dataframe_column_name_formatter: InternalDataFrameColumnNameFormatter=SimpleInternalDataFrameNameFormatter(), **kwargs: Any) -> "UnitedDataframe[CK]": ...
    @classmethod
    def from_json(cls, data: dict[str, Any], column_key_type: Type[CK], internal_dataframe_column_name_formatter: InternalDataFrameColumnNameFormatter=SimpleInternalDataFrameNameFormatter()) -> T: ...
    @classmethod
    ...
    @classmethod
    def create_empty(cls, column_keys: Sequence[CK], column_types: Mapping[CK, ColumnType], column_units_or_dimensions: Mapping[CK, Union[Unit, Dimension, None]], internal_dataframe_column_name_formatter: InternalDataFrameColumnNameFormatter=SimpleInternalDataFrameNameFormatter()) -> "UnitedDataframe[CK]": ...
    @classmethod
