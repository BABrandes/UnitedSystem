...
from ..._dataframe.column_type import LOWLEVEL_TYPE, ARRAY_TYPE, SCALAR_TYPE
import numpy as np
import pandas as pd
...
class AccessorSetitemMixin(UnitedDataframeProtocol[CK, "UnitedDataframe[CK]"]):
    ...
    @overload
    def __setitem__(self, key: CK, value: ARRAY_TYPE | Sequence[LOWLEVEL_TYPE] | Sequence[SCALAR_TYPE] | np.ndarray | pd.Series) -> None: ... # type: ignore[no-any-return]
    @overload
    def __setitem__(self, key: int, value: dict[CK, LOWLEVEL_TYPE] | dict[CK, SCALAR_TYPE]) -> None: ...
    @overload
    def __setitem__(self, key: Sequence[CK], value: ARRAY_TYPE | Sequence[LOWLEVEL_TYPE] | Sequence[SCALAR_TYPE] | np.ndarray | pd.Series) -> None: ... # type: ignore[no-any-return]
    @overload
    ...
    @overload
    def __setitem__(self, key: tuple[CK, slice], value: ARRAY_TYPE | Sequence[LOWLEVEL_TYPE] | Sequence[SCALAR_TYPE] | np.ndarray | pd.Series) -> None: ... # type: ignore[no-any-return]
    @overload
    def __setitem__(self, key: tuple[slice, CK], value: ARRAY_TYPE | Sequence[LOWLEVEL_TYPE] | Sequence[SCALAR_TYPE] | np.ndarray | pd.Series) -> None: ... # type: ignore[no-any-return]
    @overload
